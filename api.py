...
from config import OWNER_IDS, ANTHROPIC_API_KEY
...
async def ai_ask(request: web.Request):
    ...
    try:
        async with _aiohttp.ClientSession() as session:
            async with session.post("https://api.anthropic.com/v1/messages", json=payload, headers=headers, timeout=_aiohttp.ClientTimeout(total=30)) as resp:
    ...
    answer_parts = [block.get("text", "") for block in data.get("content", []) if block.get("type") == "text"]
