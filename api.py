...
def _station_for_request(request: web.Request, tg_id: str) -> str:
    """Владелец может указать ?station_id=..., оператор всегда привязан к своей станции."""
    if _is_owner(tg_id):
...
async def submit_shift(request: web.Request):
    ...
    # 2. Счётчики -> продано литров, выручка
    ...
    # 6. Отправляем отчёт владельцам в Telegram — обязательно должен дойти без ошибок
    bot = request.app["bot"]
...
async def get_reports(request: web.Request):
    """Отчёты за выбранную владельцем дату (и опционально станцию). Архив — 6 месяцев."""
    tg_id = request.query.get("tg_id")
    if not _is_owner(tg_id):
        return web.json_response({"error": "not_authorized"}, status=403)

    date = request.query.get("date")  # 'YYYY-MM-DD'
    station_id = request.query.get("station_id")  # опционально — если пусто, все станции
...
async def resolve_pending(request: web.Request):
    ...
    if approve:
        ...
        try:
            ...
            from handlers.shift import operator_keyboard
            from locales import t as _t
            await bot.send_message(
                target_id,
                _t("operator_menu", operator_lang),
                reply_markup=operator_keyboard(operator_lang),
            )
        except Exception:
            pass
    else:
...
async def set_initial_counters(request: web.Request):
    """Первый рабочий день: владелец вводит числа с колонок А/Б/В/Г.
    Разрешено ровно один раз — дальше сервер откажет."""
    tg_id = request.query.get("tg_id")
    if not _is_owner(tg_id):
        return web.json_response({"error": "not_authorized"}, status=403)
    body = await request.json()
...
async def update_settings(request: web.Request):
    """Название станции, названия котлов, теги счётчиков, цены — НЕ показания счётчиков."""
    tg_id = request.query.get("tg_id")
    if not _is_owner(tg_id):
        return web.json_response({"error": "not_authorized"}, status=403)
    body = await request.json()
...
async def ai_ask(request: web.Request):
    """Вопросы владельца на естественном языке — отвечает строго на его языке,
    опираясь на реальные данные станций (отчёты, показания, цены)."""
    tg_id = request.query.get("tg_id")
    if not _is_owner(tg_id):
        return web.json_response({"error": "not_authorized"}, status=403)
    ...
    stations = storage.get_stations()
    recent_shifts = storage.get_shifts()[-100:]  # последние 100 смен по всем станциям
...
async def get_operators(request: web.Request):
    tg_id = request.query.get("tg_id")
    if not _is_owner(tg_id):
        return web.json_response({"error": "not_authorized"}, status=403)
    station_id = request.query.get("station_id")
    ops = storage.get_operators()
    out = []
    for op_tg_id, op in ops.items():
        if station_id and op.get("station_id") != station_id:
            continue
        out.append({"tg_id": op_tg_id, "name": op["name"], "phone": op["phone"], "station_id": op.get("station_id")})
...
def create_app(bot) -> web.Application:
    ...
    app.router.add_get("/api/operators", get_operators)
