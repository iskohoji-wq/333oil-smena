...
бот здесь просто открывает его (инлайн-кнопкой в сообщении, это
надёжно передаёт данные пользователя даже на iOS) и дальше принимает
уже готовый отчёт через HTTP API (см. api.py), не через диалог в чате.
"""
