...
async def send_owner_menu(message: Message, lang: str = None):
    ...
    # постоянная кнопка в углу чата (рядом со скрепкой) — надёжно передаёт
    # данные пользователя, в отличие от обычной кнопки клавиатуры на iOS
    if OWNER_WEBAPP_URL and tg_id:
        try:
            await message.bot.set_chat_menu_button(
                chat_id=tg_id,
                menu_button=MenuButtonWebApp(text=t("owner_menu", lang), web_app=WebAppInfo(url=OWNER_WEBAPP_URL)),
            )
        except Exception:
            pass
