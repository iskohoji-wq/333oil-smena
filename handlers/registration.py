...
- После одобрения оператору сразу же приходит кнопка входа в приложение —
  не нужно писать /start заново.
"""
...
async def got_name(message: Message, state: FSMContext, bot: Bot):
    ...
    # каждому владельцу — уведомление и кнопки строго на ЕГО языке
